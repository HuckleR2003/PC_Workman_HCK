...
try:
    from hck_gpt.data import live_sensors as _live_sensors
except ImportError:
    _live_sensors = None
...
def _badge_p(pct: float) -> tuple:
    ...
    if pct >= 85:  return "CRIT",   _CCR
    if pct >= 60:  return "HIGH",   _CYL
    return "OK", _COK
...
def _build_trapezoid_header(parent, text, color):
    """Dark bordeaux-to-silver gradient trapezoid section header."""
    canvas = tk.Canvas(parent, bg="#090d20", height=24, highlightthickness=0)
    ...
    def _draw(event=None):
        ...
        for y in range(h):
            ...
            gc = int(g1 + (g2 - g1) * t)
            bc = int(b1 + (b2 - b1) * t)
            x0 = int(skew * (h - y) / h)   # trapezoid x-left at this y
            x1 = w - x0
            canvas.create_line(x0, y, x1, y, fill=f"#{rc:02x}{gc:02x}{bc:02x}")
        # Section-colour accent lines: top edge + diagonals
        canvas.create_line(skew, 0, w - skew, 0, fill=color, width=2)
